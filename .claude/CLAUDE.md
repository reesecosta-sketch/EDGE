...
### Live +EV pipeline (model-free line-shopping) — LIVE & multi-sport
- `worker/live.py --all --write` searches **MLB/Golf/NASCAR/Soccer/NFL** (exact keys for MLB=baseball_mlb & NFL=americanfootball_nfl — group-matching wrongly grabbed KBO/NCAAF; golf/nascar via active-outright discovery; preferred soccer leagues). De-vig each book → cross-book no-vig **consensus** fair line → flag prices beating it → write to Supabase. N-way EV (2-way ML, 3-way soccer, N-way outrights). Guardrails: drop >20% EV artifacts + longshot outright noise (fair<3% / >+3000). 14 tests green. Verified live: 11 clean bets across golf/nfl/soccer.
- **Daily cron:** `.github/workflows/daily-odds.yml` (13:00 UTC) runs `--all --write` using lean `worker/requirements-live.txt`. Needs repo secrets `ODDS_API_KEY` + `DATABASE_URL`. **GOTCHA:** GitHub runners are IPv4-only but Supabase direct `db.<ref>.supabase.co:5432` is IPv6 — the CI `DATABASE_URL` secret must use the **Session pooler** string (`postgres.<ref>@aws-0-<region>.pooler.supabase.com`). Local `.env` direct connection is fine (has IPv6). Worker's `_pg_params` parses either.
- This is honest +EV (soft-book mispricing vs. market), NOT a predictive model — the model/CLV gate is still the separate big project. Live demo board is now fully live (the golf seed demo was replaced).
